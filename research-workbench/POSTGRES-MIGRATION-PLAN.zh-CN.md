## PostgreSQL 迁移实施计划
日期：2026-09-08。状态：设计完成，待实施；未启动 PostgreSQL、未修改现有数据、未切换运行后端。
...
本机 docker.exe 可用，但 Docker Desktop Linux Engine 当前不可连接；未在 PATH 中找到 psql。这只影响启动与联调，不影响模型及迁移工具开发。
## 架构决定
...
每个阶段单独形成可审阅 commit；不得以“导入数量相同”代替完整性验证。当前文档只是计划，以上能力尚未实现。
## 迁移校验与恢复
...
下一步具体实施从阶段 1 开始：环境启动、数据保全与恢复演练，然后进入模型迁移。原始证据及研究成果优先保全，现有交易模块不参与此次数据库切换。
