# Entropy Arbitrage — archived learning demo

[中文](README.zh-CN.md) · [Research workspace](../../README.md)

Status: learning demo. This source still contains real order execution; it is not a simulator. The root workspace does not start it. Historical parameters, fees and SDK instructions below are reference material, not verified current configuration.

Run `npm test` for offline tests or `npm run help` for the command reference. The optional `npm run record -- --symbol SYMBOL --hedge HEDGE` JS entry always uses read-only recording. Install `requirements.txt` and pytest separately for tests. All commands below are relative to this strategy directory.
...
cd entropy-arb/strategies/entropy-arbitrage
