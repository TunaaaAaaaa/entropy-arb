# Entropy Arbitrage：学习 demo
[English](README.md) · [返回研究总项目](../../README.zh-CN.md)
状态：学习 demo。保留源码供阅读与离线测试，原有源码仍有真实下单能力，并不是模拟盘。根项目的日常命令不会启动它。下方保留的参数、费率和 SDK 安装说明属于历史参考，尚未按当前市场重新核验。
本地通过 JS 使用：npm test 运行离线测试；npm run help 查看程序参数；可选 npm run record -- --symbol SYMBOL --hedge HEDGE 始终使用只读采集模式。测试需单独安装 requirements.txt 与 pytest。以下历史命令均在本策略目录执行。
